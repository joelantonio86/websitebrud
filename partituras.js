    ...
    function createWidget(musica) {
        ...
        widget.innerHTML = `
            <div class="pr-header" onclick="prToggle(this)">
                ...
                <button class="pr-btn-all" onclick="event.stopPropagation(); prDownloadAll(this)">Baixar todos PDFs</button>
            </div>
    ...
    function renderInstrumentos(widget) {
        ...
        if (musica.hasMP3) {
            ...
            html += `<div class="pr-item pr-full-row">
                ...
                <audio controls preload="none"><source src="${url}" type="audio/mpeg"></audio>
                <a href="${url}" download class="pr-btn-mp3">Baixar MP3</a>
            </div>`;
        }
        ...
        musica.instrumentos.forEach(inst => {
            ...
            const mKey = (san === 'lira1' || san === 'lira2') ? 'lira' : san;
            const hasMp3 = musica.mp3Instruments && musica.mp3Instruments.includes(mKey);
            ...
            html += `<div class="pr-item" data-name="${inst.toLowerCase()}">
                ...
                ${hasMp3 ? `<audio controls preload="none"><source src="${MP3_BASE_URL}${musica.folder}/${musica.folder}${mKey}.mp3" type="audio/mpeg"></audio>` : '<div style="flex:1"></div>'}
                <div class="pr-links">
                    <a href="${pdf}" target="_blank" class="pr-btn-file pr-pdf">PDF</a>
                    <a href="${sib}" download class="pr-btn-file pr-sib">SIB</a>
                </div>
            </div>`;
        });
